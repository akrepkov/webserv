...
void	EventPoll::updateEventList( void )
{
	...
	while (!_pollfdsToRemoveQueue.empty()) {
        ...
        auto it = std::find_if(_pollfds.begin(), _pollfds.end(), [&](const pollfd& p) {
			return p.fd == fdToRemove.fd && p.events == fdToRemove.eventType; });
/*
[&](const pollfd& p) { return p.fd == fdToRemove.fd && p.events == fdToRemove.eventType; }:
The lambda function defines the search condition or predicate used by std::find_if
The lambda returns true if both p.fd (file descriptor in the current element) matches fdToRemove.fd and p.events matches fdToRemove.eventType.
*/
        if (it != _pollfds.end()) {
            _pollfds.erase(it);
